...
mpz_class ChooseReversed(mpz_class e, mpz_class f) {
	mpz_class x;
...
vector<uint8_t> Crack(const vector<mpz_class>& s, mpz_class y, mpz_class p, mpz_class m, mpz_class k) {
	const int charcount = 4;
	mpz_class v = FastDegree(y, k, p) * m % p;
	v = ChooseReversed(v, p);

	vector<uint8_t> vect;
	for (size_t t = 0; t < s.size() - 1; t += 2) {
		mpz_class v_ = s[t + 1];

		mpz_class M = (v_ * m * v) % p;

		for (int i = 0; i < charcount / 4; i++) {
			for (int j = 0; j < 4; j++)
				vect.push_back((mpz_class(M >> 32 * (charcount / 4 - i - 1)).get_ui() >> (8 * (3 - j))) & 0xff);
		}
	}
	return vect;
}
...
int main() {
	...
	mpz_class k = Random512bit() & ((mpz_class(1) << 511) - 1);

	cout << "Генерация k" << endl;
	while (!IsCoPrime(k, p - 1))
		k += 1;

	string raw_text = "THERE'RE NO SECRETS ";

	vector<uint8_t> raw_text_vector;
	copy(raw_text.begin(), raw_text.end(), back_inserter(raw_text_vector));

	mpz_class KNOWN_WORD = raw_text_vector[0];
	KNOWN_WORD = (KNOWN_WORD << 8) | raw_text_vector[1];
	KNOWN_WORD = (KNOWN_WORD << 8) | raw_text_vector[2];
	KNOWN_WORD = (KNOWN_WORD << 8) | raw_text_vector[3];

	vector<mpz_class> crypt_text_vector = Encrypt(raw_text_vector, y, g, p, k);
	vector<uint8_t> hacked_text_vector = Crack(crypt_text_vector, y, p, KNOWN_WORD, k);
	...
	cout << "Hack3d text: " << endl;
	cout << "\tPlain text: ";
	for (auto i : hacked_text_vector)
		cout << i;
	cout << endl;
