...
#define GMP_NEG_CAST(T,x) (~((T)((x) + 1) - 1) + 1)
...
#define gmp_ctz(count, x) do {						\
    mp_limb_t __ctz_x = (x);						\
    unsigned __ctz_c = 0;						\
    gmp_clz (__ctz_c, __ctz_x & (~__ctz_x + 1));				\
...
#define gmp_udiv_qrnnd_preinv(q, r, nh, nl, d, di)			\
  do {									\
    ...
    _r = (nl) - _qh * (d);						\
    _mask = ~((mp_limb_t) (_r > _ql)) + 1; /* both > and >= are OK */		\
    _qh += _mask;							\
    _r += _mask & (d);							\
    if (_r >= (d))							\
      {									\
