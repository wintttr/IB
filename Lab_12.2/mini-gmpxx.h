...
inline void __mpz_set_si_safe(mpz_ptr p, long l)
{
  if(l < 0)
  {
    __mpz_set_ui_safe(p, ~(static_cast<unsigned long>(l)) + 1);
    mpz_neg(p, p);
  }
  else
...
struct __gmp_binary_plus
{
  ...
  static void eval(mpz_ptr z, mpz_srcptr w, signed long int l)
  {
    if (l >= 0)
      ...
      mpz_sub_ui(z, w, ~(static_cast<unsigned long>(l)) + 1);
  }
...
struct __gmp_binary_minus
{
  ...
  static void eval(mpz_ptr z, mpz_srcptr w, signed long int l)
  {
    if (l >= 0)
      ...
      mpz_add_ui(z, w, ~(static_cast<unsigned long>(l)) + 1);
  }
  static void eval(mpz_ptr z, signed long int l, mpz_srcptr w)
  {
    if (l >= 0)
...
// gcc-3.3 doesn't have __builtin_ctzl. Don't bother optimizing for old gcc.
  ...
  static void eval(mpz_ptr z, mpz_srcptr w, signed long int l)
  {
    if (__GMPXX_CONSTANT_TRUE(l >= 0))
      ...
      {
        eval(z, w, ~(static_cast<unsigned long>(l)) + 1);
	mpz_neg(z, z);
      }
    else
      mpz_mul_si (z, w, l);
...
private:
  ...
  void assign_si(signed long l)
  {
    if (__GMPXX_CONSTANT_TRUE(l >= 0))
      ...
      {
	assign_ui(~static_cast<unsigned long>(l) + 1);
	mpz_neg(mp, mp);
      }
    else
      mpz_set_si(mp, l);
  ...
  void init_si(signed long l)
  {
    if (__GMPXX_CONSTANT_TRUE(l >= 0))
      ...
      {
	init_ui(~static_cast<unsigned long>(l) + 1);
	mpz_neg(mp, mp);
      }
    else
